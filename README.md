...
**검증한 앱 버전도 함께 기록합니다.** JUnit 결과에는 이 정보가 없어서, `scripts/run.sh` 가 테스트를 시작하기 전에 `adb` 로 조회해 `meta.json` 에 남기고 결과 페이지가 그 값을 읽습니다. 안드로이드 전용이며, `adb` 가 없거나 기기가 여러 대여서 대상이 정해지지 않거나 앱이 설치되어 있지 않으면 버전 칸이 비워집니다.
...
- **실행 이력 표.** 실행 단위의 결과와 통과 건수, 앱 버전, 기기, 실행 시각이 들어 있습니다.
